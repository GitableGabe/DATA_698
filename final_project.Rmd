...
voting_info_final_pivot <- voting_info_final %>%
  pivot_wider(
    ...
    names_from = year,
    values_from = c(totalvotes, cvap_est, voter_turnout, voter_turnout_dem, voter_turnout_gop, pctdiff_dem_vs_gop, rawdiff_dem_vs_gop, #pct_margin_of_victory, 
                    winning_party)
  )
...
voting_info_final %>%
  ...
  theme(plot.title = element_text(hjust = 0.0,size = 10), #, "Arial"
        #c(bottom,left,top,right)
        plot.margin=margin(0.25,0.05,0,0.25, "cm"),#plot.margin = margin(0.25,0.25,0,0.25, "cm"),
        legend.background=element_blank(),
        legend.position = "bottom",#legend.position = c(0.11, 0.1), #puts legend inside the plot
...
```{r}
# Create a histogram
ggplot(data = voting_info_final, aes(x = totalvotes)) +
  geom_density(fill = "#222222", alpha = 0.7, color = "darkgray") +
  facet_wrap(~year)+
  labs(title = "Distribution of Total Votes by Election year", x = "Total Votes", y = "Frequency") +
  theme_minimal()
```


```{r}
summary(voting_info_final_pivot$totalvotes_2008) #right skew
```

```{r}
summary(voting_info_final_pivot$totalvotes_2012) #right skew
```

```{r}
summary(voting_info_final_pivot$totalvotes_2016) #right skew
```

```{r}
summary(voting_info_final_pivot$totalvotes_2020) #right skew
```

```{r}
# Create a histogram
ggplot(data = voting_info_final, aes(x = voter_turnout_majparty)) +

...
vot_info_fin %>%
  ...
  ggtitle("Voter Count Distribution by Party")
```
...
corrplot.mixed(cor(cor_df %>% keep(is.numeric)),
               ...
               title="Correlation Heatmap: Variable Relationship",
               mar = c(0,0,1,0))
...
temp_train_conf_matrix_long <- melt(temp_train_conf_matrix_df, id.vars = "Actual", variable.name = "Predicted", value.name = "Count")
...
ggplot(temp_conf_matrix_df, aes(x = Predicted, y = Actual, fill = Freq)) +
  ...
  labs(
    ...
    subtitle = paste("Accuracy:", temp_conf_accuracy, "% | Balanced Accuracy:", temp_conf_balanced_accuracy, "%"),
    x = "Predicted Class",
    y = "Actual Class",
    fill = "Count"
  ) +
...
data_dict <- data.frame(
  Variable_Name = c(
    "state",
    "totalvotes_2008", "totalvotes_2012", "totalvotes_2016", "totalvotes_2020",
    "cvap_est_2008", "cvap_est_2012", "cvap_est_2016", "cvap_est_2020",
    "voter_turnout_2008", "voter_turnout_2012", "voter_turnout_2016", "voter_turnout_2020",
    "voter_turnout_dem_2008", "voter_turnout_dem_2012", "voter_turnout_dem_2016", "voter_turnout_dem_2020",
    "voter_turnout_gop_2008", "voter_turnout_gop_2012", "voter_turnout_gop_2016", "voter_turnout_gop_2020",
    "pctdiff_dem_vs_gop_2008", "pctdiff_dem_vs_gop_2012", "pctdiff_dem_vs_gop_2016", "pctdiff_dem_vs_gop_2020",
    "rawdiff_dem_vs_gop_2008", "rawdiff_dem_vs_gop_2012", "rawdiff_dem_vs_gop_2016", "rawdiff_dem_vs_gop_2020",
    "winning_party_2008", "winning_party_2012", "winning_party_2016", "winning_party_2020"
  ),
  Description = c(
...
```{r}
# Extract the confusion matrix from the rf_model
temp_train_conf_matrix <- rf_model2$confusion
temp_train_conf_matrix_df <- as.data.frame.matrix(temp_train_conf_matrix)
...
ggplot(temp_train_conf_matrix_long, aes(x = Predicted, y = Actual, fill = Count)) +
  ...
  scale_fill_gradient(low = "white", high = "steelblue") +
  labs(
    title = "Confusion Matrix for Training Data (Random Forest Final Model)",
    subtitle = paste("Final OOB Error Rate:", oob_error_rate, "%"),
    x = "Predicted Class",
    y = "Actual Class",
    fill = "Count"
...
ggplot(temp_conf_matrix_df, aes(x = Predicted, y = Actual, fill = Freq)) +
  ...
  scale_fill_gradient(low = "white", high = "steelblue") +
  labs(
    title = "Confusion Matrix for Test Data (Random Forest Final Model)",
    subtitle = paste("Accuracy:", temp_conf_accuracy, "% | Balanced Accuracy:", temp_conf_balanced_accuracy, "%"),
    x = "Predicted Class",
    y = "Actual Class",
    fill = "Count"
...
ggplot(ImpData2, aes(x=Var.Names, y=MeanDecreaseAccuracy)) +
  ...
  ggtitle("Feature Importance: Mean Decrease Accuracy for Random Forest Final Model")

```

The attributes with the lowest mean decrease accuracy in our second model are x2020_high_school_graduate_includes_equivalency, x2020_65_years_and_over, x2012_some_college_no_degree, voter_turnout_2008, and x2016_male.
...
ggplot(ImpData2, aes(x=Var.Names2, y=MeanDecreaseGini)) +
  ...
  ggtitle("Feature Importance: Mean Decrease Gini for Random Forest Final Model")
```

In our second model, the top 5 attributes are state, rawdiff_dem_vs_gop_2016, pctdiff_dem_vs_gop_2020, pctdiff_dem_vs_gop_2016, voter_turnout_dem_2020.
...
act_vs_res %>% 
  kableExtra::kable(caption = "Final Model Predictions vs. Actual 2024 Election Results") %>% 
...
act_vs_res %>% 
  ...
  kableExtra::kable(caption = "Predicted vs. Actual Outcomes for Swing States (2024)" ) %>% 
  kableExtra::kable_paper()
```
